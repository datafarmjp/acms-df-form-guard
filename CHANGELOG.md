...
- AI判定前に動くHoneypot bot対策を追加しました。
- 拡張アプリ管理画面で、Honeypot bot対策を全体設定として有効/無効にできるようにしました。
- Honeypot bot対策をOpenAI接続設定とは別の予備機能セクションとして表示するよう整理しました。
- Honeypot bot対策の設置用includeテンプレートを追加し、管理画面とREADMEに案内を追加しました。
- 管理画面のHoneypot設置用includeにコピーボタンを追加しました。
- READMEに公開フォームテンプレートへ設置するhoneypotチェックボックスのスニペットを追加しました。
- Honeypot bot対策をフォーム別AI判定設定から切り離し、全体設定ONかつinclude設置フォームだけで動作するよう整理しました。
- 管理画面にHoneypot設定の保存値と実効値を表示し、公開フォーム側の設定読み取りを共通設定サービスへ揃えました。
