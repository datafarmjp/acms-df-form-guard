...
- フォーム履歴に判定結果を保存
- フォーム履歴一覧で `OK / NG / ERROR` の判定結果を確認
- CSSで隠したチェック項目によるHoneypot bot対策
- フォームごとの追加判定ルールを設定
...
- フォーム履歴一覧で判定結果を見るには、フォームログを保存する
- a-blog cms コアは改修せず、既存 Hook と互換 POST で実装する

## 動作概要

- 判定タイミングは `beforeSendAutoReply`
- Honeypot bot対策に引っかかった送信は、AI判定前に送信処理を中止する
...
フォームID編集画面の基本設定で「ログを残さないようにする」を有効にした場合も、AI判定と管理者宛メール抑制は動作します。<br>
ただし、フォーム履歴一覧に `OK / NG / ERROR` の判定結果を表示できないため、運用確認が必要なフォームではログを保存する設定をおすすめします。

## Honeypot bot対策

Honeypot bot対策は、AIとは別に動く予備機能です。フォーム内に見えない確認項目を設置している場合、自動入力された迷惑送信を送信処理の前に止めます。

拡張アプリ管理画面で有効にしていても、`df_form_guard_honeypot` を設置していないフォームには影響しません。

公開フォームテンプレートの送信対象の `<form>` 内に、以下のincludeを追加してください。`<form>` の外に置くと送信データに含まれないため動作しません。

```html
@include("/extension/plugins/DF_FormGuard/template/include/honeypot.html")
```

includeの中では、通常のcheckboxをCSSで画面外に隠して出力します。`type="hidden"` ではなく、HTML上は通常のチェックボックスにします。

展開される内容は以下です。

```html
<div class="df-form-guard-honeypot" aria-hidden="true">
  <label>
...
確認画面ありのフォームでは、入力画面の `<form>` 内に置くと、チェックされた送信を確認画面へ進めず通常の入力エラーとして扱います。エラー後の修正画面ではHoneypotは未チェック表示に戻る想定です。
...
パンくずの拡張アプリ名も、`admin-topicpath` への `InjectTemplate` 差し込みで表示します。
...
- `extension/acms/POST/FormGuardLogDecisions.php`
